...
graph TB
    ...
    subgraph "Phoenix Core"
        Brain[Cerebrum Nexus<br/>Master Orchestrator]
        Memory[Memory Systems<br/>5 Layers + Vaults]
        LLM[LLM Orchestrator<br/>500+ Models]
        Context[Context Engine<br/>EQ-First Context]
    end
...
sequenceDiagram
    ...
    UI->>MasterOrch: Route Command
    
    alt Memory Query
        MasterOrch->>Memory: Retrieve Memories
        Memory-->>MasterOrch: Return Context
    end
    
    MasterOrch->>Context: Build EQ-First Context
    Context-->>MasterOrch: Context String
    
    MasterOrch->>LLM: Generate Response
    LLM-->>MasterOrch: Response Text
    
    alt Action Required
        MasterOrch->>Tools: Execute Tool
        Tools->>External: Perform Action
        External-->>Tools: Result
        Tools-->>MasterOrch: Tool Output
    end
    
    MasterOrch->>Memory: Store Interaction
    MasterOrch-->>UI: Display Response
    UI-->>User: Show Result
```
