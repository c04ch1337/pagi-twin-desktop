...
2. **Setup Config**  
   ...
   **LLM Provider Configuration:**
   - **OpenRouter (Default)**: Set `OPENROUTER_API_KEY` - Get key at https://openrouter.ai/keys
   - **Ollama (Optional)**: Set `LLM_PROVIDER=ollama`, `OLLAMA_BASE_URL`, and `OLLAMA_MODEL` for local GPU rig

3. **Build & Run**  
   ```bash
   cargo build --workspace
   cargo run --bin phoenix-web  # Start API server
   ```
...
| <img src="https://fonts.gstatic.com/s/i/short-term/release/materialsymbolsoutlined/record_voice_over/default/20px.svg" width="20" height="20" alt="Voice"> | **LLM Orchestrator** | Vocal cords (OpenRouter/Ollama) | Phoenix's voice. Supports OpenRouter (500+ models) as default, or Ollama (local GPU rig) as optional provider. Handles streaming, fallback chains, and model tier selection (Free/Floor/Nitro). |
...
graph TD
    ...
    J --> Q
    Q --> R{LLM Provider}
    R -->|OpenRouter| S1[OpenRouter API (500+ models)]
    R -->|Ollama| S2[Ollama API (Local GPU rig)]
    S1 --> T[LLM Response]
    S2 --> T
...
- Approvals → Expose via a frontend (Web/Tauri) by calling the same GitHub endpoints used by the backend agents.

#### Step 3 — Current reality

The legacy terminal UI (TUI) binary has been removed. Use [`phoenix-web`](phoenix-web/src/main.rs:1) as the runtime entrypoint.
...
## <img src="https://fonts.gstatic.com/s/i/short-term/release/materialsymbolsoutlined/sensors/default/24px.svg" width="24" height="24" alt="Multimedia"> Multimedia & Network Intelligence
...
Phoenix now has **Multimedia & Network Intelligence** capabilities — advanced sensory systems that enable continuous ambient recording, desktop capture, wireless network analysis, and multi-modal context correlation. These features transform Phoenix into a true digital twin with comprehensive environmental awareness.

### Overview

The Multimedia & Network Intelligence system provides:

**Core Capabilities:**
- **Audio Intelligence**: Continuous ambient recording, meeting transcription, voice activity detection, audio analysis
- **Desktop Capture**: Screen recording, visual data extraction (OCR), diagram analysis
- **Wireless Intelligence**: WiFi network discovery, traffic analysis, Bluetooth device detection
- **Context Correlation**: Multi-modal event alignment, temporal pattern recognition
- **Privacy Framework**: Content blurring, consent management, never-record lists
- **Hardware Detection**: Auto-detection of available audio/video/network hardware

**Status**: ![Implemented](https://img.shields.io/badge/Foundation-Complete-success) Foundation complete, ready for feature implementation

### Architecture

**Module Structure:**
- **`audio_intelligence/`**: Audio capture, transcription, and analysis
- **`desktop_capture_service/`**: Screen capture and visual data extraction
- **`wireless_sniffer/`**: WiFi and Bluetooth network intelligence
- **`context_correlation_engine/`**: Multi-modal event correlation
- **`privacy_framework/`**: Privacy controls and consent management
- **`hardware_detector/`**: Hardware auto-detection and fallback strategies

**Memory Integration:**
The sensory data maps to Phoenix's existing layered memory architecture:

| Sensory Layer | Phoenix Memory | Key Format | Purpose |
|--------------|---------------|------------|---------|
| L1 (Instant Cache) | STM | `stm:sensory:audio:{timestamp}` | 30s audio ring buffer |
| L2 (Working Memory) | WM | `wm:sensory:screen:{timestamp}` | Active screen context |
| L3 (Episodic) | EPM | `epm:sensory:session:{timestamp}` | Recorded sessions |
| L4 (Semantic) | Mind Vault | `mind:sensory:extracted:{category}` | Extracted knowledge |
| L5 (Procedural) | Body Vault | `body:sensory:settings:{key}` | Capture settings |
| L6 (Evolutionary) | LTM | `ltm:sensory:evolution:{algorithm}` | Algorithm optimization |
| L7 (Transcendent) | RFM | `rfm:sensory:pattern:{pattern_id}` | Cross-sensory patterns |

**Documentation**: Complete design document available in [`docs/MULTIMEDIA_NETWORK_INTELLIGENCE_ADDENDUM.md`](docs/MULTIMEDIA_NETWORK_INTELLIGENCE_ADDENDUM.md)

### Audio Intelligence

**Features:**
- **Continuous Ambient Recording**: Ring buffer maintains last 30 seconds of audio
- **Wake Word Detection**: Triggers recording when wake word is detected
- **Meeting Transcription**: Full meeting transcription with speaker diarization (planned)
- **Audio Analysis**: Tone, sentiment, and keyword extraction
- **Voice Activity Detection**: Automatic detection of speech vs. silence

**API Endpoints:**
- `POST /api/audio/start-ambient` - Start continuous ambient listening
- `POST /api/audio/stop-ambient` - Stop ambient listening
- `POST /api/audio/start-recording` - Start recording a session
- `POST /api/audio/stop-recording` - Stop recording and get transcript
- `GET /api/audio/status` - Get current listening/recording status

**Configuration:**
```env
AUDIO_INTELLIGENCE_ENABLED=true
AUDIO_RING_BUFFER_SIZE=30
WAKE_WORD=Phoenix
VAD_THRESHOLD=0.5
```
...
**Features:**
- **Multiple Capture Modes**: Full desktop, active window, region select, continuous low-FPS
- **Visual Data Extraction**: OCR text extraction from screenshots
- **Diagram Analysis**: Extract and analyze tables, charts, flowcharts
- **Continuous Monitoring**: Low-FPS ambient capture for context awareness

**API Endpoints:**
- `POST /api/desktop/capture` - Capture screen with specified mode
- `POST /api/desktop/extract-text` - Extract text from captured image

**Capture Modes:**
```json
{
  "mode": "full",        // Full desktop
  "mode": "window",      // Active window
  "mode": "region",      // User-defined region
...
CAPTURE_STORAGE_PATH=./data/captures
```

### Wireless Intelligence

**WiFi Analyzer:**
- Network discovery (SSIDs, signal strength, encryption)
- Traffic analysis (protocols, volumes, patterns)
- Security assessment (weak encryption, rogue APs)
- Device fingerprinting (manufacturer, OS, behavior)

**Bluetooth Sniffer:**
- Device discovery (name, class, services)
- Connection monitoring
- BLE advertising capture
- Security analysis

**API Endpoints:**
- `GET /api/wireless/wifi/networks` - Discover WiFi networks
- `GET /api/wireless/wifi/traffic?duration=60` - Analyze network traffic
- `GET /api/wireless/bluetooth/devices` - Discover Bluetooth devices

**Configuration:**
```env
WIFI_ANALYZER_ENABLED=true
BLUETOOTH_SNIFFER_ENABLED=true
```

### Context Correlation Engine

**Features:**
- **Multi-Modal Event Alignment**: Correlate audio, visual, and network events
- **Temporal Pattern Recognition**: Identify patterns across time windows
- **Cross-Sensory Patterns**: Detect correlations like "stress audio + error screen = user needs help"
- **Pattern Storage**: Store patterns in RFM (Reflexive Flame) layer for eternal memory

**Configuration:**
```env
CORRELATION_ENGINE_ENABLED=true
CORRELATION_TIME_WINDOW=60
```

### Privacy Framework

**Features:**
- **Never-Record Lists**: Configure apps/windows to never record
- **Automatic Blurring**: Blur faces, credit cards, personal documents
- **Consent Management**: Request user consent for sensitive operations
- **Retention Policies**: Automatic data purging based on configurable retention

**API Endpoints:**
- `GET /api/privacy/config` - Get privacy configuration
- `POST /api/privacy/config` - Update privacy configuration

**Configuration:**
```env
PRIVACY_FRAMEWORK_ENABLED=true
```

**Privacy Config Example:**
```json
{
  "never_record": ["password_fields", "financial_apps"],
  "blur_automatically": ["Faces", "CreditCards"],
  "require_confirmation": ["ScreenSharing", "WebcamRecording"],
  "retention_days": 30,
  "auto_delete": true
}
```
...
- **Fallback Strategies**: Select appropriate fallback when hardware unavailable
- **Device Enumeration**: List audio interfaces, cameras, WiFi adapters, Bluetooth adapters

**API Endpoints:**
- `GET /api/hardware/audio` - List audio interfaces
- `GET /api/hardware/cameras` - List camera devices

**Configuration:**
```env
HARDWARE_DETECTOR_ENABLED=true
```
...
graph TB
    ...
    B -->|Audio| C[Audio Intelligence]
    B -->|Screen| D[Desktop Capture]
    B -->|Network| E[Wireless Sniffer]
    
    C --> F[Ring Buffer<br/>STM Layer]
    C --> G[Meeting Transcript<br/>EPM Layer]
    C --> H[Audio Analysis<br/>Mind Vault]
    
    D --> I[Screen Capture<br/>WM Layer]
    D --> J[OCR Extraction<br/>Mind Vault]
    D --> K[Diagram Analysis<br/>Mind Vault]
    
    E --> L[Network Discovery]
    E --> M[Traffic Analysis]
    E --> N[Security Report]
    
    F --> O[Context Correlation Engine]
    G --> O
    I --> O
    L --> O
    
    O --> P[Pattern Recognition<br/>RFM Layer]
    P --> Q[Phoenix Context]
    Q --> R[LLM Response]
...
**Phase 1 (Immediate):**
- ✅ Foundation crates created
- 🔄 Audio capture with Whisper.cpp integration
- 🔄 Basic screen capture implementation
- 🔄 Simple WiFi scanning

**Phase 2 (Short-term):**
- Continuous ambient recording with actual audio capture
- Video recording with OpenCV
- Wireless traffic analysis with compatible hardware
- Bluetooth device discovery

**Phase 3 (Advanced):**
- Real-time multi-modal correlation
- Advanced wireless forensics
- RF spectrum analysis (with SDR)
- Predictive sensory activation

### Ethical Guardrails

All multimedia features include built-in privacy controls:
...
- **Design Document**: [`docs/MULTIMEDIA_NETWORK_INTELLIGENCE_ADDENDUM.md`](docs/MULTIMEDIA_NETWORK_INTELLIGENCE_ADDENDUM.md)
- **Memory Architecture**: [`docs/LAYERED_MEMORY_ARCHITECTURE.md`](docs/LAYERED_MEMORY_ARCHITECTURE.md)
- **Multi-Modal System**: [`docs/MULTI_MODAL_ARCHITECTURE.md`](docs/MULTI_MODAL_ARCHITECTURE.md)
- **Database Systems**: [`docs/DATABASE_SOLUTIONS_ARCHITECTURE.md`](docs/DATABASE_SOLUTIONS_ARCHITECTURE.md)
...
- **API Gateway**: Provides RESTful endpoints for API-only mode (no frontend required)
- **Integration Hub**: Coordinates all Phoenix modules and services
- **State Manager**: Maintains application state across requests

**Note**: Phoenix runs in API-only mode by default. The backend provides a unified REST API at `http://127.0.0.1:8888` for integration with any frontend or client application.
...
- **LLM Orchestrator**: Response generation (OpenRouter or Ollama)

**See**: [`docs/MASTER_ORCHESTRATION_ARCHITECTURE.md`](docs/MASTER_ORCHESTRATION_ARCHITECTURE.md) for complete documentation.

## <img src="https://fonts.gstatic.com/s/i/short-term/release/materialsymbolsoutlined/record_voice_over/default/24px.svg" width="24" height="24" alt="LLM"> LLM Provider Configuration

Phoenix AGI supports two LLM providers: **OpenRouter** (default) and **Ollama** (optional local GPU rig).

### OpenRouter (Default)

OpenRouter provides access to 500+ AI models from various providers (OpenAI, Anthropic, Google, Meta, etc.) through a single unified API.

**Configuration:**
```bash
# Set provider (default, can be omitted)
...
- Model tier shortcuts (`:free`, `:floor`, `:nitro`)
- Streaming support

### Ollama (Optional - Local GPU Rig)

Ollama allows you to run models locally on your GPU rig, providing privacy and cost savings for local inference.
...
# Required: Your Ollama server URL
OLLAMA_BASE_URL=http://192.168.1.100:11434

# Required: Model name (check with: ollama list)
OLLAMA_MODEL=llama3
```

**Requirements:**
- Ollama installed and running on your GPU rig
- Network access from Phoenix AGI to the Ollama server
- `OLLAMA_HOST=0.0.0.0` set on the Ollama server (to allow network connections)

**Testing Connection:**
```bash
# From Phoenix AGI machine, test connectivity:
curl http://192.168.1.100:11434/api/tags
```

**See**: [`docs/LLM_SOLUTION_ARCHITECTURE.md`](docs/LLM_SOLUTION_ARCHITECTURE.md) for complete LLM architecture documentation.
...
| Variable | Description | Default | Source Location |
|----------|-------------|---------|----------------|
| `LLM_PROVIDER` ![Optional](https://img.shields.io/badge/Optional-green) | LLM provider selection: `"openrouter"` (default) or `"ollama"` | `"openrouter"` | `llm_orchestrator/src/lib.rs:30` |
| `OPENROUTER_API_KEY` ![Conditional](https://img.shields.io/badge/Conditional-yellow) | **Required if using OpenRouter.** OpenRouter API key for LLM access. Get at https://openrouter.ai/keys | None | `llm_orchestrator/src/lib.rs:188` |
| `OLLAMA_BASE_URL` ![Optional](https://img.shields.io/badge/Optional-green) | **Required if using Ollama.** Ollama server base URL (e.g., `http://192.168.1.100:11434`) | `http://192.168.1.100:11434` | `llm_orchestrator/src/lib.rs:210` |
| `OLLAMA_MODEL` ![Optional](https://img.shields.io/badge/Optional-green) | **Required if using Ollama.** Ollama model name (e.g., `llama3`, `mistral`, `codellama`) | `llama3` | `llm_orchestrator/src/lib.rs:212` |
| `DEFAULT_LLM_MODEL` ![Optional](https://img.shields.io/badge/Optional-green) | Default LLM model to use (OpenRouter format: `anthropic/claude-4-sonnet:free`, `openai/gpt-4o-mini`) | `openai/gpt-4o-mini` | `llm_orchestrator/src/lib.rs:204` |
| `FALLBACK_LLM_MODEL` ![Optional](https://img.shields.io/badge/Optional-green) | Backup model if primary fails (OpenRouter only) | `openai/gpt-4o-mini` | `llm_orchestrator/src/lib.rs:208` |
...
| `OPENAI_API_KEY` ![Optional](https://img.shields.io/badge/Optional-green) | OpenAI API key (not used - use OpenRouter instead) | None | `.env.example` |
| `ANTHROPIC_API_KEY` ![Optional](https://img.shields.io/badge/Optional-green) | Anthropic API key (not used - use OpenRouter instead) | None | `.env.example` |
| `GROK_API_KEY` ![Optional](https://img.shields.io/badge/Optional-green) | Grok API key (not used - use OpenRouter instead) | None | `.env.example` |
...
# 5. Build Phoenix (API server)
cargo build --release --bin phoenix-web

# 6. Run Phoenix
./target/release/phoenix-web
```
...
The recording command surface is frontend-agnostic; wire these commands to your UI (Web/Tauri) and route them to the backend.
